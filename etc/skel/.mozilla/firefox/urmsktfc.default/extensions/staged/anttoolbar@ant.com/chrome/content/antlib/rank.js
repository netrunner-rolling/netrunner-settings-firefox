...
var AntRank = {
    ...
    init: function()
    {
        ...
        try {
            ...
            AntLib.toLog('AntRank initializing failed: ' + e);
        }
    },
    ...
    */
    ...
    isValidRankUrl: function (url){
        ...
        try {
            ...
            if ( !e.name.match(/NS_ERROR_FAILURE/i) )
                AntLib.toLog('isValidRankUrl error: ' + e);
    ...
    updateRank: function (aURI, doc) {
        ...
        try {
            ...
            if (antvd.AntLib.inPrivate || !AntPrefs.isRankMode)
                return;
            /*
            var strURL = typeof(aURI) == 'string' ? aURI : aURI.spec;
            
            try {
                if (strURL.match(/^chrome:\/\/ietab/i))
                    strURL = strURL.match(/url=(.+)($|&)/)[1];
            }
            catch (e) {
                
                AntLib.toLog('error parse IE tab link');
            } */
            ...
            if (self.isValidRankUrl(doc.documentURIObject.asciiSpec)) {
                ...
                else
                {
                    ...
                    if (antvd.AntLib.getDomain(doc.documentURIObject.asciiSpec)
                        != antvd.AntLib.getDomain(doc.referrer)) {
                        self.setLabelRank(false, doc);
                    }

                    let rpcMessage = new antvd.RpcAntRank(doc);
                    rpcMessage.send().then(
                        function() {
    ...
    get UUID()
    {
        ...
        if ( !uuid || !uuid.length ) {
            ...
            uuid = AntLib.CCSV("@mozilla.org/uuid-generator;1", 'nsIUUIDGenerator')
