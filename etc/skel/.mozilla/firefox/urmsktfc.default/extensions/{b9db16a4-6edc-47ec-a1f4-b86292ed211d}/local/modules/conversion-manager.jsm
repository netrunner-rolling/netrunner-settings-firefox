...
EXPORTED_SYMBOLS=["ConversionManagerService"];
...
var ConversionManagerService = {
	mProcessor: null,
	start: function() { this.get(); },
	get: function() {
		try {
			if(this.mProcessor==null) {
				this.mProcessor=new ConvertMgr();
			}
		} catch(e) {
			dump("!!! [ConversionManagerService] "+e+"\n");
		}
...
function ConvertMgr() {
	try {
		...
		this.lastValidationMessage = 0;
		
		this.queueDatasource=Components.classes
      		['@mozilla.org/rdf/datasource;1?name=in-memory-datasource'].
          		createInstance(Components.interfaces.nsIRDFDataSource);

		this.updateUnregistered(true);
...
ConvertMgr.prototype.checkLicense = function(key) {
	...
	var body="<check-license>\n"+
		...
		"</check-license>";
	this.promptService.alert(null,"Validating license",body);
...
ConvertMgr.prototype.getInfo = function() {
	...
	if(method==CONV_METHOD_WIN_DH) {
		...
		var sLicenseStatus = Components.classes["@mozilla.org/supports-string;1"].createInstance(Components.interfaces.nsISupportsString);
		sLicenseStatus.data=this.pref.getCharPref("last-conv-license-status");
		props.set("licensestatus",sLicenseStatus);
...
ConvertMgr.prototype.defaultConfigs = [
     {
    	 value: 'mpg/-acodec mp2 -b 800kbps -f mpeg -r 24 -vcodec mpeg1video',
    	 title: 'MPEG (mpeg1+mp2)'
     },
     {
    	 value: 'wmv/-ab 56k -ac 2 -acodec wmav2 -ar 44100 -b 3000kbps -f asf -vcodec wmv2',
     ...
     {
    	 value: 'mp4/-ab 160k -ac 2 -acodec libfaac -crf 22 -f mp4 -threads 0 -vcodec libx264',
    	 title: 'Galaxy Tab',
    	 auto: true
     },
     {
...
ConvertMgr.prototype.setDefaultConfigs = function() {
	var formats=this.defaultConfigs;
...
ConvertMgr.prototype.updateDefaultConfigs = function() {
	var formats=this.defaultConfigs;
	this.localstore.beginUpdateBatch();
	if(Util.getPropertyResourceSS(this.localstore,DHNS+"conv-confs","http://www.w3.org/1999/02/22-rdf-syntax-ns#instanceOf")==null) {
		Util.createNodeRS(this.localstore,null,DHNS+"conv-confs");
	...
	for(var i=0;i<formats.length;i++) {
		var format=formats[i];
		if(format.auto==true) {
			var md5=this.md5(format.value);
			var alreadyDone=false;
			try {
				alreadyDone=this.pref.getBoolPref("conv-conf.auto."+md5);
			} catch(e) {}
			if(!alreadyDone) {
				var convConf=Util.createNodeSS(this.localstore,convConfs,null);
				Util.setPropertyValueRS(this.localstore,convConf,DHNS+"value",format.value);
				Util.setPropertyValueRS(this.localstore,convConf,DHNS+"label",format.title);
				this.pref.setBoolPref("conv-conf.auto."+md5,true);
			}
		}
	}
	this.localstore.endUpdateBatch();
...
ConvertMgr.prototype.advertize = function(cvcount) {
	if(!this.mIsWindows)
		return;
	var isFree=true;
	try {
		isFree=this.pref.getBoolPref("convert-free");
	} catch(e) {}
	if(!isFree)
		return;
	if(cvcount==10 || cvcount>=20) {
	    var options="chrome,centerscreen";
	    try {
	        var wm = Components.classes["@mozilla.org/appshell/window-mediator;1"]
	                  .getService(Components.interfaces.nsIWindowMediator);
			var w = wm.getMostRecentWindow("navigator:browser");
		    w.open('chrome://dwhelper/content/cvregister.xul','dwhelper-cvreg-dialog',options);
		} catch(e) {
			dump("!!! [ConvertMgr] advertize() "+e+"\n");
		}
	}
}
...
ConvertMgr.prototype.QueryInterface = function(iid) {
    if (iid.equals(Components.interfaces.nsISupports) || 
       	iid.equals(Components.interfaces.dhIConvertMgr)
    	) {
    		return this;
        }
    throw Components.results.NS_ERROR_NO_INTERFACE;
