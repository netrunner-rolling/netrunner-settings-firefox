...
var AntPrefs = 
{
    ...
    init: function()
    {
        ...
        if ( self.isRankMode ) {
            ...
            var dMode = (self.displayMode == 'addonsbar')
                    ? "statusbar"
                    : self.displayMode;
            if ( dMode == 'ctoolbar' ) {
    ...
    manageDisplayMode: function (aForce) {
        AntLib.toLog("manageDisplayMode");
        var self = AntPrefs;
        ...
        var mode = (displayMode == 'addonsbar')
                ? 'statusbar'
                : displayMode;
        ...
        case 'ctoolbar':
            ...
            if (aForce) {
                AntLib.toLog("Adding compact toolbar buttons");
        ...
        case 'toolbar':
            if (self.toolbar != null) {
                ...
                AntLib.ob('navigator-toolbox')
                    .insertBefore(self.toolbar, pt.nextSibling);
                self.toolbar = null;
            }
            break;
        default:
    ...
    manageFlvDir: function ()
    {
        ...
        if (self.flvDir)
        {
            /** @type nsIFile */ let dir = null;
            try {
                dir = FileUtils.File(self.flvDir);
                if (!dir.exists() || !dir.isDirectory()) {
                    AntLib.toLog(
                        "[PREFS]: Download directory referenced by flvDir"
                            + " doesn't exist:"
                            + "\nPath: " + self.flvDir);

                    dir.createUnique(
                        AntLib.CI("nsIFile").DIRECTORY_TYPE
        ...
        try {
            if (!isPathValid) {
                self.flvDir
                = FileUtils.getDir("DfltDwnld", ["Ant Videos"], true).path;
            }
        } catch (ex) {
            AntLib.logError("[PREFS]: Failed to create a default download directory"
                            , ex);
        }
    },
    ...
    */
    ...
    get converterPath()
    {
        ...
        var prefStorage = Components.classes["@mozilla.org/preferences-service;1"]
